...
export default function GamePage() {
    ...
    const handleKickPlayer = useCallback((kickedPlayerId: string) => {
    ...
    const handlePlayerJoined = useCallback((newPlayer: GameRoom['players'][0]) => {
        setRoom(prevRoom => prevRoom ? {
            ...prevRoom,
    ...
    const handleRoomClosed = useCallback((reason: 'host-left') => {
        if (reason === 'host-left') {
    ...
    if (isConnecting) {
        return (
            <div
                className="min-h-screen bg-linear-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center p-4">
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <header className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 mb-4 sm:mb-6">
                    ...
                    <div className="text-center order-first sm:order-0 w-full sm:w-auto">
                        ...
                        <span
                            className="text-white font-mono font-bold ml-1 sm:ml-2 text-sm sm:text-base">{roomCode}</span>
                    </div>
                ...
                {error && (
                    <div
                        className="mb-4 p-3 bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl text-red-600 dark:text-red-400 text-sm text-center">
                        {error}
                    </div>
                )}
